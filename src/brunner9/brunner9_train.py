...
def load_BCI42_data(preprocessed_data_path: str, data_file: str) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load EEG data and labels from .npy files for BCI Competition IV dataset 2a.

    Args:
        preprocessed_data_path: Path to the dataset directory
        data_file: Base filename (without extension)

    Returns:
        Tuple containing EEG data array and corresponding labels
    """
    data_path = os.path.join(preprocessed_data_path, data_file + '_data.npy')
    label_path = os.path.join(preprocessed_data_path, data_file + '_label.npy')
...
def load_all_data(
    ...
    Args:
        raw_data_path: Path to the dataset folder
    ...
    if not os.path.exists(preprocessed_data_path):
        ...
        preprocess_competition(raw_data_path, preprocessed_data_path)
    ...
    for subId in range(1, 10):
        ...
        train_data, train_labels = load_BCI42_data(preprocessed_data_path, train_datafile)
        test_data, test_labels = load_BCI42_data(preprocessed_data_path, test_datafile)
